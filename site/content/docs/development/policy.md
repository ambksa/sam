...
* **Roles**: Define specific capabilities (allowed destinations and services).
  ...
  * `allowed_services`: Defines the application-level tools or endpoints a user can access. Services use a strict `type://name` convention (e.g., `mcp://db-agent` or `inference://openrouter`).
    ...
    * **The service is the unit of authorization — deliberately.** Mesh policy does not filter individual MCP tools inside a service. The mesh authorizes *names* (see the architecture's "a mesh name is the service namespace" decision), and a registered service is the operator's exposure decision: registering it offers its whole tool surface, exactly as the backend defines it. To offer different privilege tiers, split them into services — register a read-only backend as `mcp://db-reader` and a mutating one as `mcp://db-writer` (see the node configuration example in §6.2) and grant them to different roles. If a third-party MCP server bundles tools you do not want to offer, front it with a backend that exposes only the tools you mean to publish; curating the tool surface is the server registration's job, not the token's.
